...
MCP Gateway 是一个基于 Go 语言开发的轻量但高可用的网关服务，旨在让个人和企业在 MCP (Model Control Protocol) 浪潮下可以无痛将存量的 API 服务（RESTful, gRPC 等）通过配置的方式转成 MCP-Server。

非常存粹的目的和功能特性：
- 🌐 **平台中立**：不管是物理机、虚拟机、ECS、K8s 等场景都可以无痛接入，无需对基础设施动手
- 🔁 **支持多种协议的转换**：RESTful、gRPC 等都可以通过配置的方式转成 MCP-Server
- ⚡️ **追求性能和轻松的多副本高可用**：轻量但不对可用及性能妥协
- 🧭 **简单直观的管理页面**：拒绝学习和运维成本
