...
func TestSSETransport_EarlyFailures(t *testing.T) {
    ...
    if tr.IsRunning() { t.Fatalf("unexpected running state") }
    if err := tr.Stop(context.Background()); err != nil { t.Fatalf("stop idle: %v", err) }
    if _, err := tr.FetchTools(context.Background()); err == nil { t.Fatalf("expected fetch error") }
    ...
    if err == nil { t.Fatalf("expected call tool error") }
}
